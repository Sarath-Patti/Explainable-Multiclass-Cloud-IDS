"""Random Forest Model Training & Evaluation Module for Explainable-Multiclass-Cloud-IDS.

This module loads preprocessed datasets and artifacts, performs efficient hyperparameter
tuning using RandomizedSearchCV with 3-Fold Stratified Cross-Validation on a representative
stratified tuning subset (250k samples), selects the optimal hyperparameters, and trains
the final Random Forest model on the complete training dataset (1.76M samples).
...
logger = logging.getLogger("RandomForestBaseline")
...
def load_preprocessed_data() -> tuple[
    ...
    """Loads preprocessed datasets and artifacts.
    ...
    logger.info("Loading preprocessing artifacts (label_encoder.pkl, class_weights.pkl)...")
    label_encoder = joblib.load(MODELS_DIR / "label_encoder.pkl")
    class_weights = joblib.load(MODELS_DIR / "class_weights.pkl")

    return X_train, X_valid, X_test, y_train, y_valid, y_test, label_encoder, class_weights
...
def tune_hyperparameters(
    ...
    Args:
        ...
        class_weights: Loaded class weights dictionary.
        tuning_size: Size of the stratified tuning subset.

    Returns:
        tuple containing:
            - best_params: Dict of best hyperparameters.
            - best_cv_score: Best Macro F1 cross-validation score on the tuning subset.
    ...
    logger.info("Executing RandomizedSearchCV on tuning subset (3-Fold CV, 5 iterations)...")
...
def train_final_model(
    ...
    """Trains the final Random Forest model on the complete training dataset.
    ...
    Returns:
        tuple containing:
            - final_model: Fitted RandomForestClassifier on complete training data.
            - train_time: Duration of full model training in seconds.
    """
    logger.info("Instantiating final Random Forest model with selected best parameters...")
    ...
    logger.info(f"Retraining final Random Forest model on COMPLETE training dataset ({len(X_train):,} samples)...")
    start_train = time.time()
    final_rf.fit(X_train, y_train)
    train_time = time.time() - start_train

    logger.info(f"Full training completed in {train_time:.2f} seconds.")
    logger.info(f"Out-of-Bag (OOB) Score: {final_rf.oob_score_:.6f}")

    return final_rf, train_time
...
def evaluate_model(
    ...
    """Evaluates the model on a dataset split and calculates detailed metrics.
...
def generate_plots(
    ...
    plt.title("Random Forest Baseline - Test Confusion Matrix", fontsize=14, pad=15)
    plt.xlabel("Predicted Class Label", fontsize=12)
    plt.ylabel("True Class Label", fontsize=12)
    plt.xticks(rotation=45, ha="right")
    plt.yticks(rotation=0)
    plt.tight_layout()
    plt.savefig(plots_dir / "rf_confusion_matrix.png", dpi=300)
    plt.close()
    ...
    plt.title("Random Forest Baseline - Top 20 Feature Importances", fontsize=14, pad=15)
    ...
    plt.title("Random Forest Baseline - One-vs-Rest ROC Curves", fontsize=14, pad=15)
    plt.legend(loc="lower right", fontsize=9, frameon=True)
    plt.tight_layout()
    plt.savefig(plots_dir / "rf_roc_curves.png", dpi=300)
    plt.close()
    ...
    plt.title("Random Forest Baseline - Precision-Recall Curves", fontsize=14, pad=15)
    plt.legend(loc="lower left", fontsize=9, frameon=True)
    plt.tight_layout()
    plt.savefig(plots_dir / "rf_precision_recall.png", dpi=300)
    plt.close()
...
def save_artifacts_and_metrics(
    ...
    with open(METRICS_DIR / "rf_best_params.json", "w", encoding="utf-8") as f:
    ...
    with open(METRICS_DIR / "rf_metrics.json", "w", encoding="utf-8") as f:
    ...
    clf_report_df.to_csv(METRICS_DIR / "rf_classification_report.csv", index=True)

    # 5. Save confusion matrix CSV
    cm = confusion_matrix(y_test, y_test_pred)
    cm_df = pd.DataFrame(cm, index=class_names, columns=class_names)
    cm_df.to_csv(METRICS_DIR / "rf_confusion_matrix.csv", index=True)
...
def generate_report(
    ...
    """
    logger.info("Compiling Random Forest baseline report...")
    ...
    lines = [
        "==================================================",
        "RANDOM FOREST BASELINE MODEL PERFORMANCE REPORT",
        "==================================================",
        "",
        ...
        f"OOB Score (Out-Of-Bag): {model.oob_score_:.6f}",
...
def main() -> None:
    """Main execution function to orchestrate the Random Forest baseline pipeline."""
    LOGS_DIR.mkdir(parents=True, exist_ok=True)
    log_file = LOGS_DIR / "random_forest.log"
    ...
    logger.info("Starting Random Forest Baseline Training & Evaluation pipeline...")
    ...
    try:
        ...
        best_params, best_cv_score, tuning_time, tuning_samples = tune_hyperparameters(
            X_train, y_train, class_weights, tuning_size=250000
        )

        # 3. Retrain final Random Forest model on complete training dataset (1.76M samples)
        final_rf, train_time = train_final_model(
            X_train, y_train, class_weights, best_params
        )

        # 4. Evaluate on validation and test sets
        logger.info("Evaluating Random Forest model on validation split...")
        val_metrics, val_inf_time, _, _ = evaluate_model(final_rf, X_valid, y_valid, class_names)

        logger.info("Evaluating Random Forest model on test split...")
        test_metrics, test_inf_time, y_test_pred, y_test_proba = evaluate_model(
            final_rf, X_test, y_test, class_names
        )

        # 5. Generate plots
        generate_plots(
            final_rf, X_test, y_test, y_test_pred, y_test_proba, class_names, PLOTS_DIR
        )

        # 6. Save artifacts and metrics
        save_artifacts_and_metrics(
            final_rf, best_params, best_cv_score, tuning_time, train_time,
        ...
        with open(REPORTS_DIR / "random_forest_report.txt", "w", encoding="utf-8") as f:
            f.write(report_text)

        logger.info("Random Forest Baseline pipeline completed successfully.")

    except Exception as e:
        logger.exception("An error occurred during Random Forest baseline pipeline execution:")
        raise e
